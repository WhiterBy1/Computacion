...
class Producto(Base):
    ...
    @classmethod
    def obtener_todos(cls, session, order_by: str = None, ascending: bool = True, 
                     offset: int = 0, limit: int = None) -> List['Producto']:
        """
    ...
    @classmethod
    def contar_todos(cls, session) -> int:
        """
    ...
    def buscar(cls, session, 
               ...
               ascending: bool = True,
               offset: int = 0,
               limit: int = None) -> List['Producto']:
        """
        Searches products based on multiple criteria with sorting and pagination.
    ...
    @classmethod
    def contar_busqueda(cls, session, 
                       texto: Optional[str] = None, 
                       precio_min: Optional[float] = None, 
                       precio_max: Optional[float] = None,
                       stock_min: Optional[int] = None, 
                       stock_max: Optional[int] = None) -> int:
        """
        Counts the total number of products matching the search criteria.
...
class SistemaInventario:
    ...
    def buscar_productos(self, texto: Optional[str] = None, 
                        ...
                        ascending: bool = True,
                        pagina: int = 1,
                        por_pagina: int = 20) -> Tuple[List[Producto], int, int]:
        """
        Searches products based on multiple criteria with sorting and pagination.
        ...
        Args:
            ...
            por_pagina: Number of records per page
            
        Returns:
            Tuple with list of products, total count, and total pages
        """
        try:
            # Calculate offset
            ...
            # Get total count
            total = Producto.contar_busqueda(
                self.session,
                texto=texto,
            ...
            return productos, total, total_paginas
    ...
    def obtener_todos_productos(self, order_by: str = None, ascending: bool = True,
                              pagina: int = 1, por_pagina: int = 20) -> Tuple[List[Producto], int, int]:
        """
        Gets all products from inventory with optional sorting and pagination.
        ...
        try:
            ...
            # Get total count
            total = Producto.contar_todos(self.session)
...
class EstiloUI:
    ...
    def aplicar_estilo(root):
        ...
        style.configure("Paginacion.TFrame", 
                       background=EstiloUI.COLOR_FONDO)
        style.configure("Paginacion.TButton", 
                       padding=2)
        style.configure("PaginaActual.TButton", 
                       background=EstiloUI.COLOR_SECUNDARIO)
...
class MensajeExito(tk.Toplevel):
    """Ventana emergente para mostrar mensajes de éxito."""
    ...
    def __init__(self, parent, titulo, mensaje):
        ...
        lbl_icono = ttk.Label(frame_icono, text="✓", font=("Helvetica", 24), 
                             foreground=EstiloUI.COLOR_EXITO, style="TLabel")
        lbl_icono.pack(side="left", padx=(0, 10))
        
        lbl_mensaje = ttk.Label(frame_icono, text=mensaje, wraplength=250, 
                               justify="left", style="TLabel")
        lbl_mensaje.pack(side="left", fill="x", expand=True)
        ...
        btn_cerrar = ttk.Button(frame_botones, text="Aceptar", 
                               command=self.destroy)
        btn_cerrar.pack(side="right")
...
class ControlPaginacion(ttk.Frame):
    """Control de paginación para tablas."""
    
    def __init__(self, parent, pagina_actual=1, total_paginas=1, items_por_pagina=20, 
                 cambiar_pagina_callback=None, cambiar_items_callback=None):
        super().__init__(parent, style="Paginacion.TFrame")
        ...
        self.cambiar_items_callback = cambiar_items_callback
    ...
    def crear_controles(self):
        ...
        # Items per page
        frame_items = ttk.Frame(self, style="Paginacion.TFrame")
        frame_items.pack(side="left", padx=5)
        
        lbl_items = ttk.Label(frame_items, text="Mostrar:", style="TLabel")
        lbl_items.pack(side="left", padx=(0, 5))
        
        self.combo_items = ttk.Combobox(frame_items, width=5, state="readonly")
        self.combo_items['values'] = [10, 20, 50, 100]
        self.combo_items.set(self.items_por_pagina)
        self.combo_items.pack(side="left")
        
        if self.cambiar_items_callback:
            self.combo_items.bind("<<ComboboxSelected>>", self._cambiar_items)
        
        # Page navigation
        frame_nav = ttk.Frame(self, style="Paginacion.TFrame")
        frame_nav.pack(side="right", padx=5)
        
        # First page button
        self.btn_primera = ttk.Button(frame_nav, text="«", width=2, 
                                     command=lambda: self._cambiar_pagina(1),
                                     style="Paginacion.TButton")
        self.btn_primera.pack(side="left", padx=2)
        
        # Previous page button
        self.btn_anterior = ttk.Button(frame_nav, text="‹", width=2, 
                                      command=lambda: self._cambiar_pagina(self.pagina_actual - 1),
                                      style="Paginacion.TButton")
        self.btn_anterior.pack(side="left", padx=2)
        
        # Page indicator
        self.lbl_pagina = ttk.Label(frame_nav, 
                                   text=f"Página {self.pagina_actual} de {self.total_paginas}",
                                   style="TLabel")
        self.lbl_pagina.pack(side="left", padx=5)
        
        # Next page button
        self.btn_siguiente = ttk.Button(frame_nav, text="›", width=2, 
                                       command=lambda: self._cambiar_pagina(self.pagina_actual + 1),
                                       style="Paginacion.TButton")
        self.btn_siguiente.pack(side="left", padx=2)
        
        # Last page button
        self.btn_ultima = ttk.Button(frame_nav, text="»", width=2, 
                                    command=lambda: self._cambiar_pagina(self.total_paginas),
                                    style="Paginacion.TButton")
    ...
    def _cambiar_pagina(self, nueva_pagina):
        """Changes the current page."""
        if nueva_pagina < 1 or nueva_pagina > self.total_paginas:
            return
        
        self.pagina_actual = nueva_pagina
        self._actualizar_estado_botones()
        
        if self.cambiar_pagina_callback:
            self.cambiar_pagina_callback(nueva_pagina)
    
    def _cambiar_items(self, event=None):
        """Changes the number of items per page."""
        nuevo_items = int(self.combo_items.get())
        if nuevo_items == self.items_por_pagina:
            return
        
        self.items_por_pagina = nuevo_items
        
        if self.cambiar_items_callback:
            self.cambiar_items_callback(nuevo_items)
    ...
    def _actualizar_estado_botones(self):
        ...
        if self.pagina_actual <= 1:
            self.btn_primera.config(state="disabled")
            self.btn_anterior.config(state="disabled")
        else:
            self.btn_primera.config(state="normal")
    ...
    def actualizar(self, pagina_actual, total_paginas):
        """Updates pagination control with new values."""
        self.pagina_actual = pagina_actual
        self.total_paginas = max(1, total_paginas)
        self._actualizar_estado_botones()
...
class AplicacionInventario:
    ...
    def __init__(self, root):
        ...
        # Para Linux/Mac se usaría: self.root.attributes('-zoomed', True)

        self.pagina_actual_listado = 1
        self.pagina_actual_busqueda = 1
    ...
    def configurar_tab_listado(self):
        ...
        # Pagination control
        self.paginacion_listado = ControlPaginacion(
            self.tab_listado, 
            pagina_actual=1, 
            total_paginas=1,
            items_por_pagina=self.items_por_pagina,
            cambiar_pagina_callback=self.cambiar_pagina_listado,
            cambiar_items_callback=self.cambiar_items_por_pagina_listado
        )
        self.paginacion_listado.pack(fill="x", padx=10, pady=5)
    ...
    def cambiar_pagina_listado(self, nueva_pagina):
        """Changes the current page in the product listing."""
        self.pagina_actual_listado = nueva_pagina
        self.cargar_productos()
    
    def cambiar_items_por_pagina_listado(self, nuevo_items):
        """Changes the number of items per page in the product listing."""
        self.items_por_pagina = nuevo_items
        self.pagina_actual_listado = 1  # Reset to first page
        self.cargar_productos()
    ...
    def configurar_tab_busqueda(self):
        ...
        self.paginacion_busqueda = ControlPaginacion(
            self.tab_busqueda, 
            pagina_actual=1, 
            total_paginas=1,
            items_por_pagina=self.items_por_pagina,
            cambiar_pagina_callback=self.cambiar_pagina_busqueda,
            cambiar_items_callback=self.cambiar_items_por_pagina_busqueda
        )
        self.paginacion_busqueda.pack(fill="x", padx=10, pady=5)
        
        # Action buttons for search results
        marco_botones_res = ttk.Frame(self.tab_busqueda, style="TFrame")
        marco_botones_res.pack(fill="x", padx=10, pady=10)
        
        btn_editar_res = ttk.Button(marco_botones_res, text="Editar Producto", 
                                   command=self.editar_producto_seleccionado_busqueda)
        btn_editar_res.pack(side="left", padx=5)
        
        btn_eliminar_res = ttk.Button(marco_botones_res, text="Eliminar Producto", 
                                     command=self.eliminar_producto_seleccionado_busqueda)
        btn_eliminar_res.pack(side="left", padx=5)
        
        # Double click to edit in search results
        self.tabla_resultados.bind("<Double-1>", lambda e: self.editar_producto_seleccionado_busqueda())
    
    def cambiar_pagina_busqueda(self, nueva_pagina):
        """Changes the current page in the search results."""
        self.pagina_actual_busqueda = nueva_pagina
        self.buscar_productos()
    
    def cambiar_items_por_pagina_busqueda(self, nuevo_items):
        """Changes the number of items per page in the search results."""
        self.items_por_pagina = nuevo_items
        self.pagina_actual_busqueda = 1  # Reset to first page
        self.buscar_productos()
    
    def editar_producto_seleccionado_busqueda(self):
        """Loads data from the selected product in search results into the form."""
        ...
        item = self.tabla_resultados.item(seleccion[0])
        valores = item["values"]
    ...
    def eliminar_producto_seleccionado_busqueda(self):
        """Deletes the selected product from search results."""
        seleccion = self.tabla_resultados.selection()
        if not seleccion:
    ...
    def cargar_productos(self):
        """Loads products into the table with current sorting and pagination."""
        # Clear table
        for item in self.tabla_productos.get_children():
            self.tabla_productos.delete(item)
        
        # Get products with current sort settings and pagination
        productos, total, total_paginas = self.sistema.obtener_todos_productos(
            order_by=self.sort_column, 
            ascending=self.sort_ascending,
            pagina=self.pagina_actual_listado,
            por_pagina=self.items_por_pagina
        )
        ...
        self.paginacion_listado.actualizar(self.pagina_actual_listado, total_paginas)
        
        # Update status
        self.actualizar_estado(f"Se cargaron {len(productos)} productos de un total de {total}.")
    ...
    def guardar_producto(self):
        ...
        if id_producto:  # Edit existing product
            ...
            exito, mensaje, producto = self.sistema.agregar_producto(
                nombre, descripcion, precio_float, stock_int
            )
        
        # Show result
        if exito:
            # Mejorado: Mostrar mensaje de éxito en ventana emergente
            if not id_producto:  # Si es un nuevo producto
                MensajeExito(self.root, "Producto Agregado", 
                           f"El producto '{nombre}' ha sido agregado correctamente.")
    ...
    def limpiar_busqueda(self):
        ...
        self.var_busqueda_orden.set("nombre") 
        ...
        # Reset pagination
        self.pagina_actual_busqueda = 1
        self.paginacion_busqueda.actualizar(1, 1)
    
    def buscar_productos(self):
        """Searches products according to specified criteria with pagination."""
        # Get search criteria
        ...
        resultados, total, total_paginas = self.sistema.buscar_productos(
            ...
            order_by=order_by,
            ascending=ascending,
            pagina=self.pagina_actual_busqueda,
            por_pagina=self.items_por_pagina
        )
        ...
        # Update pagination control
        self.paginacion_busqueda.actualizar(self.pagina_actual_busqueda, total_paginas)
        
        # Update status
        self.actualizar_estado(f"Se encontraron {total} productos. Mostrando página {self.pagina_actual_busqueda} de {total_paginas}.")
